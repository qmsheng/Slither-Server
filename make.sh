...
if [ $OSTYPE == "UBUNTU" ] ; then
    apt-get install -y build-essential libpcre3-dev libssl-dev git-core unzip libev-dev
elif [ $OSTYPE == "CENTOS" ]; then
    yum groupinstall -y "Development Tools"
    yum install -y pcre-devel zlib-devel openssl-devel unzip libev-dev
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ $UID -eq 0 ]; then
            sudo -u $SUDO_USER brew install pcre openssl libev
            sudo -u $SUDO_USER brew link openssl --force
        else
            brew install pcre openssl libev
...
cp src/redis-check-rdb $DEST_BIN_DIR/redis/bin
...
# install luarocks
echo ""
echo -e "[\033[32mINSTALL\033[0m] luarocks"

cd $BUILD_DIR
tar zxf luarocks-$LUAROCKS_VER.tar.gz
cd luarocks-$LUAROCKS_VER
./configure --prefix=$DEST_BIN_DIR/openresty/luajit \
	--with-lua=$DEST_BIN_DIR/openresty/luajit/ \
	--lua-suffix=jit-2.1.0-beta2 \
	--with-lua-include=$DEST_BIN_DIR/openresty/luajit/include/luajit-2.1
...
# echo ""
# echo -e "[\033[32mINSTALL\033[0m] lpack,lbase64"
# cd $BUILD_DIR
# echo "_GBC_CORE_ROOT_/bin/openresty/luajit/bin/luarocks install lpack-20070629-1.src.rock" > lpack.sh
#
# echo "_GBC_CORE_ROOT_/bin/openresty/luajit/bin/luarocks install lbase64-20120820-1.src.rock" >> lpack.sh
# echo "_GBC_CORE_ROOT_/bin/openresty/luajit/bin/luarocks install lua-websockets" >> lpack.sh
# echo "_GBC_CORE_ROOT_/bin/openresty/luajit/bin/luarocks install lua-ev" >> lpack.sh
# echo "_GBC_CORE_ROOT_/bin/openresty/luajit/bin/luarocks install xxtea" >> lpack.sh
#
# $SED_BIN "s#_GBC_CORE_ROOT_#$DEST_DIR#g" lpack.sh
# $SED_BIN "s#lua ./codegen.lua#$DEST_BIN_DIR/openresty/luajit/bin/lua ./codegen.lua#g" lpack.sh
# sh lpack.sh
